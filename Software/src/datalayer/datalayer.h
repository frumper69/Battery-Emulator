...
struct DATALAYER_SYSTEM_STATUS_TYPE {
  ...
  /** True if a battery reports its own contactor state via CAN (rather than BE driving external
   *  relays itself). Lets the webserver show the detailed contactors_engaged-based status even when
   *  contactor_control_enabled is false, instead of the generic "not fully controlled" disclaimer.
   *  Set once by such a battery's setup(); contactors_engaged writes from battery code must still be
   *  guarded with !contactor_control_enabled, same as dc_bus_live below, so the GPIO contactor state
   *  machine remains the single writer whenever it's actually running. */
  bool battery_reports_contactor_state = false;
